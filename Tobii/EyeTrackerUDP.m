...
%
% Mohammad Ihsaan 3-3-2023: Added code to create a udp server and send
% the x and y coords over, commented with initials ' - M.I'
%
% Minor tweaks by Chris Miall 20-8-2018 to invert data in final plot, so it
% matches the display screen (assuming 1920x1080 resolution display)
%
...
% create udp server - M.I
h = udp('127.0.0.1', 5665); 
fopen(h);
...
while ishandle(hFig) % check if figure still open
    ...
    if ~isempty(x)
        ...
        % set(gca, 'XLim',[0 max([x(end,1) get(gca,'XLim')])], 'YLim',[0 max([x(end,2) get(gca,'YLim')])] ); %removed this self-scaling Chris Miall 20-8-18
        drawnow();
        
        % add data to store
        x_all = [x_all; x];
        
        % concatenate x and y - M.I
        x_coord = round(x(end,1));
        y_coord = round(x(end,2));
        coords = [num2str(x_coord), ',', num2str(y_coord)];
        ...
        % send coords over udp - M.I
        fwrite(h, coords);
    else 
        set(hTxt, 'String', 'Waiting for data');
    end
    
    pause(1/25); % run at 25 Hz - M.I
end
...
%close all                             -- COMEMETN
%plot(xx(:,1), xx(:,2), '-o');         -- DEORFAO
%set(gca, 'xlim',[0 1920], 'ylim',[0 1080], 'Ydir','reverse'); %inverted the Y-axis Chris Miall 20-8-18
%axis equal;  %correct scaling Chris Miall 20-8-18
...
% fps check
fprintf('\nApprox FPS (Gaze): %1.2f Hz\n', 1./(mean(diff(x_all(:,3)))/1000));
fprintf('\nApprox FPS (Eye Pos): %1.2f Hz\n', 1./(mean(diff(x_all(:,12)))/1000)); %just checking Chris Miall 20-8-18

% close udp connection - M.I
fclose(h);